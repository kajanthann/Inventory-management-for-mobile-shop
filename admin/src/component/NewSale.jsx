...
import {
  ...
  FaCheck, FaMinus, FaUndo, FaReceipt, FaPrint,        // ← ADD FaPrint
} from "react-icons/fa";
...
const toastStyle = { background: "#1a1a1a", color: "#fff", border: "1px solid #2a2a2a", borderRadius: "10px", fontSize: "13px" };
const warnToast  = (msg) => toast(msg,       { icon: "⚠️", style: { ...toastStyle, borderLeft: "3px solid #eab308" } });
const errorToast = (msg) => toast.error(msg, { style: { ...toastStyle, borderLeft: "3px solid #b00000" }, iconTheme: { primary: "#b00000", secondary: "#1a1a1a" } });

const CART_KEY = "smartspider_cart";

const NewSale = ({ products, loadingProducts, onSaleComplete, onViewHistory }) => {
  ...
  useEffect(() => { sessionStorage.setItem(CART_KEY, JSON.stringify(cartItems)); }, [cartItems]);
  ...
  const addToCart = (product) => {
    if (product.qty <= 0) return warnToast(`${product.name} is out of stock`);
    setLocalProducts((prev) => prev.map((p) => p._id === product._id ? { ...p, qty: p.qty - 1 } : p));
    setCartItems((prev) => {
      const exists = prev.find((i) => i.productId === product._id);
      if (exists) {
        if (exists.qty >= exists.maxStock) { warnToast(`Only ${exists.maxStock} units available`); return prev; }
        const newQty = exists.qty + 1;
        return prev.map((i) => i.productId === product._id ? { ...i, qty: newQty, subtotal: newQty * i.unitPrice - i.discount } : i);
      }
      return [...prev, { productId: product._id, name: product.name, imei: product.imei || "", qty: 1, unitPrice: product.price, discount: 0, subtotal: product.price, maxStock: product.qty }];
    });
  };
  ...
  const updateQty = (productId, delta) => {
    setCartItems((prev) =>
      prev.map((i) => {
        ...
        if (newQty > i.maxStock) { warnToast(`Only ${i.maxStock} units available`); return i; }
        return { ...i, qty: newQty, subtotal: newQty * i.unitPrice - i.discount };
      })
    );
  };
  ...
  const updateItemDiscount = (productId, val) => {
    setCartItems((prev) =>
      prev.map((i) => i.productId === productId
        ? { ...i, discount: Number(val) || 0, subtotal: Math.max(0, i.qty * i.unitPrice - (Number(val) || 0)) }
        : i
      )
    );
  };
  ...
  const clearCart = () => { setCartItems([]); setGlobalDiscount(0); setAmountPaid(""); sessionStorage.removeItem(CART_KEY); };
  ...
  const handleCompleteSale = async () => {
    ...
    if (paid < grandTotal) return errorToast("Amount paid is less than grand total");
    if (submitting) return;
    if (!customer.name.trim())    return errorToast("Customer name is required");
    if (!customer.cashier.trim()) return errorToast("Cashier name is required");
    if (!customer.phone.trim())   return errorToast("Phone number is required");

    setSubmitting(true);
    const invoiceData = { customer: { ...customer }, items: [...cartItems], paymentMethod, subtotal, discount, grandTotal, amountPaid: paid, change, status: "completed" };
    const result = await onSaleComplete(invoiceData);
    if (result?.success) {
      setSuccessInvoice({ ...invoiceData, invoiceNo: result.sale.invoiceNo, date: result.sale.date, time: result.sale.time });
      clearCart();
  ...
  const handleViewHistory = () => {
    const inv = successInvoice;
    setSuccessInvoice(null);
    toast.success(`Sale ${inv.invoiceNo} saved!`, {
      ...
      <div className="flex lg:hidden gap-1 p-1 rounded-xl border border-gray-200 dark:border-[#1f1f1f] w-fit bg-gray-100 dark:bg-[rgba(10,10,10,0.6)]">
        {[
          ...
          <button key={p.key} onClick={() => setMobilePanel(p.key)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${mobilePanel === p.key ? "bg-[#b00000] text-white" : "bg-transparent text-gray-500 hover:text-gray-900 dark:hover:text-white"}`}>
            {p.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-5 gap-4">
          ...
          <div className={cardCls}>
            <div className="px-3 sm:px-4 py-3 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input placeholder="Search product by name or IMEI..." value={productSearch}
                  onChange={(e) => setProductSearch(e.target.value)} className={`${inputCls} pl-8`} />
              </div>
              <span className="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">{filteredProducts.length} items</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 max-h-[260px] sm:max-h-[300px] overflow-y-auto">
              {loadingProducts ? (
                [...Array(6)].map((_, i) => <div key={i} className="h-16 rounded-lg bg-gray-100 dark:bg-[#1a1a1a] animate-pulse" />)
                ...
                const isLow    = p.qty > 0 && p.qty <= 5;
                const isOut    = p.qty === 0;
                return (
                  <button key={p._id}
                    onClick={() => { addToCart(p); setMobilePanel("invoice"); }}
                    disabled={isOut}
                    style={{ borderColor: inCart ? "rgba(176,0,0,0.5)" : undefined }}
                    className="flex items-start justify-between p-3 rounded-lg border text-left transition disabled:opacity-40 disabled:cursor-not-allowed border-gray-200 dark:border-[#1f1f1f] hover:border-[#b00000]/40 bg-gray-50 dark:bg-[rgba(10,10,10,0.7)]">
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-semibold text-gray-900 dark:text-white leading-tight truncate">{p.name}</div>
                      <div className="text-[10px] text-gray-400 dark:text-gray-500 font-mono mt-0.5 truncate">{p.imei || p.category || ""}</div>
                      {inCart && <div className="text-[10px] text-[#b00000] dark:text-[#e05050] mt-0.5 font-medium">{inCart.qty} in cart</div>}
                    </div>
                    <div className="text-right ml-2 flex-shrink-0">
                      <div className="text-xs font-bold text-[#b00000] dark:text-[#e05050]">{Rs(p.price)}</div>
                      <div className={`text-[10px] font-mono mt-0.5 ${isOut ? "text-red-500" : isLow ? "text-yellow-500" : "text-gray-400 dark:text-gray-500"}`}>
                        {isOut ? "Out of stock" : isLow ? `Low: ${p.qty}` : `Qty: ${p.qty}`}
                      </div>
                    </div>
                  </button>
                );
          ...
          <div className={cardCls}>
            <div className="px-3 sm:px-4 py-3 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-400 dark:text-gray-500">{cartItems.length} item{cartItems.length !== 1 ? "s" : ""}</span>
                {cartItems.length > 0 && (
                  <button onClick={clearCart} className="text-xs text-gray-400 hover:text-red-500 transition flex items-center gap-1">
            ...
            {cartItems.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm" style={{ minWidth: "480px" }}>
                  ...
                  <tbody>
                    {cartItems.map((item) => (
                      <tr key={item.productId} className="border-t border-gray-100 dark:border-[#1a1a1a] hover:bg-gray-50 dark:hover:bg-white/[0.01]">
                        <td className="px-3 sm:px-4 py-2">
                          <div className="text-xs font-medium text-gray-900 dark:text-white">{item.name}</div>
                          {item.imei && <div className="text-[10px] text-gray-400 dark:text-gray-500 font-mono">{item.imei}</div>}
                          <div className="text-[10px] text-gray-400 dark:text-gray-500">Max: {item.maxStock}</div>
                        </td>
                        <td className="px-3 sm:px-4 py-2">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => updateQty(item.productId, -1)}
                              className="w-5 h-5 rounded flex items-center justify-center border border-gray-300 dark:border-[#2a2a2a] text-gray-500 hover:border-[#b00000] hover:text-red-500 transition">
                              <FaMinus size={8} />
                            </button>
                            <span className="text-xs font-semibold w-5 text-center text-gray-900 dark:text-white">{item.qty}</span>
                            <button onClick={() => updateQty(item.productId, 1)}
                              disabled={item.qty >= item.maxStock}
                              className={`w-5 h-5 rounded flex items-center justify-center border transition ${item.qty >= item.maxStock ? "border-gray-200 dark:border-[#1f1f1f] text-gray-300 dark:text-gray-700 cursor-not-allowed" : "border-gray-300 dark:border-[#2a2a2a] text-gray-500 hover:border-[#b00000] hover:text-green-500"}`}>
                              <FaPlus size={8} />
                            </button>
                          </div>
                        </td>
                        ...
                        <td className="px-3 sm:px-4 py-2 text-right">
                          <input type="number" value={item.discount || ""} placeholder="0" min="0"
                        ...
                        <td className="px-3 sm:px-4 py-2">
                          <button onClick={() => removeFromCart(item.productId)} className="p-1 text-gray-400 hover:text-red-500 transition">
                            <FaTrash size={10} />
                          </button>
                        </td>
                      </tr>
        ...
        <div className={`lg:col-span-2 space-y-4 ${mobilePanel === "products" ? "hidden lg:block" : "block"}`}>
          ...
          <div className={`${cardCls} p-3 sm:p-4 space-y-3`}>
            ...
            <input placeholder="Customer name *" value={customer.name} required
              onChange={(e) => setCustomer({ ...customer, name: e.target.value })} className={inputCls} />
            <input placeholder="Cashier name *" value={customer.cashier} required
              onChange={(e) => setCustomer({ ...customer, cashier: e.target.value })} className={inputCls} />
            <input placeholder="Phone number *" value={customer.phone} required
              onChange={(e) => setCustomer({ ...customer, phone: e.target.value })} className={inputCls} />
          </div>
          ...
          <div className={`${cardCls} p-3 sm:p-4 space-y-3`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {PAYMENT_METHODS.map((m) => (
                  <button key={m} onClick={() => setPaymentMethod(m)}
                    className={`py-1.5 rounded-lg text-xs font-medium border transition ${paymentMethod === m ? "border-[#b00000] text-[#b00000] dark:text-[#e05050] bg-[#b00000]/10" : "border-gray-300 dark:border-[#1f1f1f] text-gray-500 bg-gray-50 dark:bg-[#0a0a0a]"}`}>
                    {m}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-2 pt-1">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-700 dark:text-gray-300">{Rs(subtotal)}</span>
              </div>
              {cartItems.some((i) => i.discount > 0) && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Item Discounts</span>
                  <span className="text-yellow-500 dark:text-yellow-400">- {Rs(cartItems.reduce((s, i) => s + i.discount, 0))}</span>
                </div>
              )}
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500">Extra Discount</span>
                <input type="number" placeholder="0" min="0" value={globalDiscount || ""}
                  onChange={(e) => setGlobalDiscount(e.target.value)}
                  className="w-24 sm:w-28 px-2 py-1 rounded text-xs text-right outline-none transition border focus:border-[#b00000] bg-gray-100 dark:bg-[#0a0a0a] border-gray-300 dark:border-[#1f1f1f] text-yellow-600 dark:text-yellow-400" />
              </div>
            ...
            <div>
              ...
              <input type="number" placeholder="Enter amount" min="0" value={amountPaid}
                onChange={(e) => setAmountPaid(e.target.value)} className={inputCls} />
            </div>
            {paid > 0 && grandTotal > 0 && (
              <div className={`flex justify-between text-sm px-3 py-2 rounded-lg border ${paid < grandTotal ? "border-red-500/20 bg-red-500/5" : "border-green-500/20 bg-green-500/5"}`}>
                <span className="text-gray-500">{paid < grandTotal ? "Remaining" : "Change"}</span>
                <span className={`font-bold ${paid < grandTotal ? "text-red-500" : "text-green-600 dark:text-green-400"}`}>
                  {paid < grandTotal ? `- ${Rs(grandTotal - paid)}` : Rs(change)}
                </span>
              </div>
            )}
            <button onClick={handleCompleteSale} disabled={cartItems.length === 0 || submitting}
              className="w-full py-3 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2 transition bg-[#b00000] hover:bg-[#8b0000] disabled:opacity-40 disabled:cursor-not-allowed">
              {submitting ? (
            ...
            <button onClick={clearCart} disabled={submitting}
              className="w-full py-2 rounded-xl text-sm flex items-center justify-center gap-2 transition border border-gray-300 dark:border-[#1f1f1f] text-gray-500 hover:text-red-500 bg-transparent disabled:opacity-40">
      ...
      {successInvoice && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="w-full max-w-[420px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] overflow-hidden bg-white dark:bg-[#111111]">
            ...
            <div className="p-4 sm:p-5 space-y-3 max-h-[45vh] overflow-y-auto">
