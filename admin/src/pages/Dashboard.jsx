      ...
      <div className="space-y-4 animate-pulse">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-24 rounded-xl bg-gray-200 dark:bg-[#1a1a1a]"
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-52 rounded-xl bg-gray-200 dark:bg-[#1a1a1a]"
            />
          ))}
        </div>
      </div>
    ...
    <div className="text-gray-900 dark:text-white space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          {
            icon: <FaDollarSign />,
            label: `${kpiLabel} Revenue`,
            value: Rs(kpi.revenue),
            sub: `${kpi.sales} sales`,
            color: "#22c55e",
          ...
          },
          {
            ...
            sub: `${data?.totalProducts || 0} products`,
            color: "#f59e0b",
          ...
          },
          {
            ...
            value: data?.repairStats?.pending || 0,
            sub: `${data?.repairStats?.total || 0} total jobs`,
            color: "#3b82f6",
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span style={{ color: s.color, opacity: 0.8, fontSize: 13 }}>
                {s.icon}
              </span>
            </div>
                ...
                <PieChart>
                  ...
                  >
                    {data.paymentChart.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={PAY_COLORS[entry.name] || CAT_COLORS[i]}
                        stroke="transparent"
                      />
                    ))}
                  </Pie>
                  <Tooltip
                    formatter={(v, n, p) => [`${v} sales`, p.payload.name]}
                    contentStyle={{
                      background: "#111",
                      border: "1px solid #1f1f1f",
                      borderRadius: 8,
                ...
                {data.paymentChart.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-1.5">
                    <div
                      ...
                      style={{
                        background: PAY_COLORS[d.name] || CAT_COLORS[i],
                      }}
                    />
                    <span className="text-[10px] text-gray-500 truncate">
                      {d.name}
                    </span>
                    <span className="text-[10px] font-semibold text-gray-900 dark:text-white ml-auto">
                      {d.value}
                    </span>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className={`${cardCls} p-3 sm:p-4`}>
          <div className="text-sm font-semibold text-gray-900 dark:text-white mb-1 flex items-center gap-2">
            <FaWrench size={11} className="text-[#b00000]" /> Repair Jobs — 6
            Months
          </div>
          <div className="text-[10px] text-gray-500 mb-3">
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            {[
              ...
              {
                label: "Done",
                value: data?.repairStats?.done || 0,
                color: "text-green-500 bg-green-500/10 border-green-500/20",
              },
              {
                label: "Returned",
                value: data?.repairStats?.returned || 0,
                color: "text-blue-500 bg-blue-500/10 border-blue-500/20",
              },
            ].map((s) => (
              <div
                key={s.label}
                className={`text-center px-2 py-1.5 rounded-lg border text-[10px] font-medium ${s.color}`}
              >
        ...
        <div className={`${cardCls} overflow-hidden`}>
          <div className="px-3 sm:px-4 py-3 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
            <div className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <FaShoppingCart size={12} className="text-[#b00000]" /> Recent
              Sales
            </div>
            <span className="text-[10px] text-gray-400">
              {data?.kpi?.allTime?.sales || 0} total
            </span>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-[#1a1a1a]">
            {(data?.recentSales || []).length === 0 ? (
              ...
              data.recentSales.map((s) => (
                <div
                  key={s._id}
                  className="px-3 sm:px-4 py-2.5 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-white/[0.02] transition"
                >
                  <div className="min-w-0 flex-1 mr-2">
                    <div className="text-[11px] font-mono font-bold text-[#b00000] dark:text-[#e05050]">
                      {s.invoiceNo}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0 space-y-0.5">
                    ...
                    <div
                      className={`text-[9px] px-1.5 py-0.5 rounded-full border inline-block ${statusColor(s.status)}`}
                    >
                      {s.status}
                    </div>
                  </div>
        ...
        <div className="space-y-3 sm:col-span-2 lg:col-span-1">
            ...
            <div className="p-2 space-y-1.5 max-h-[160px] overflow-y-auto">
              {(data?.lowStock || []).length === 0 ? (
                ...
                data.lowStock.map((p) => (
                  <div
                    key={p._id}
                    className="flex items-center justify-between px-2.5 py-1.5 rounded-lg border border-yellow-500/10 bg-yellow-500/[0.04]"
                  >
                    <div className="min-w-0 flex-1 mr-2">
          ...
          <div className={`${cardCls} overflow-hidden`}>
            <div className="px-3 sm:px-4 py-2.5 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
              ...
              <span className="text-[10px] text-gray-400">
                {data?.repairStats?.pending || 0} jobs
              </span>
            </div>
            <div className="p-2 space-y-1.5 max-h-[160px] overflow-y-auto">
              {(data?.pendingRepairs || []).length === 0 ? (
                ...
                data.pendingRepairs.map((r) => (
                  <div
                    key={r._id}
                    className="px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-[#1f1f1f] bg-gray-50 dark:bg-[rgba(10,10,10,0.5)]"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-gray-900 dark:text-white truncate mr-2">
                        {r.brand} {r.model}
                      </span>
