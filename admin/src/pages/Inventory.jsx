...
const Inventory = () => {
  const { products, fetchProducts, axiosInstance } = useContext(AppContext);
  ...
  // ── Skeleton (Dashboard-style full blocks) ──
  if (loading) {
    return (
      <div className="space-y-4 sm:space-y-5 animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-24 rounded-xl bg-gray-200 dark:bg-[#1a1a1a]" />
          ))}
        </div>
        {/* Toolbar */}
        <div className="h-10 rounded-xl bg-gray-200 dark:bg-[#1a1a1a]" />
        {/* Table */}
        <div className="h-[420px] rounded-xl bg-gray-200 dark:bg-[#1a1a1a]" />
      </div>
    );
  }

  const openAdd = () => { setForm(emptyForm); setEditItem(null); setOpen(true); };
  const openEdit = (p) => { setForm({ ...p }); setEditItem(p._id); setOpen(true); };
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        name: form.name, imei: form.imei, category: form.category,
        qty: Number(form.qty), cost: Number(form.cost),
        price: Number(form.price), description: form.description,
      };
      if (editItem) {
        confirmToast("Update this product?", async () => {
          const { data } = await axiosInstance.put(`/api/products/${editItem}`, payload);
          if (data.success) { toast.success("Product updated"); fetchProducts(); closeModal(); }
          else toast.error(data.message);
        });
        return;
      } else {
        const { data } = await axiosInstance.post("/api/products/add", payload);
        if (data.success) { toast.success("Product added"); fetchProducts(); }
        else toast.error(data.message);
      }
      closeModal();
    } catch (err) {
          ...
          <table className="w-full text-sm" style={{ minWidth: "700px" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  <tr key={p._id} className="border-t border-gray-100 dark:border-[#1a1a1a] hover:bg-gray-50 dark:hover:bg-white/[0.02] transition">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1.5">
                        <button onClick={() => setViewItem(p)} className="p-1.5 rounded border border-blue-500/20 text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 transition">
                          <FaEye size={11} />
                        </button>
                        <button onClick={() => openEdit(p)} className="p-1.5 rounded border border-yellow-500/20 text-yellow-500 dark:text-yellow-400 hover:bg-yellow-500/10 transition">
                          <FaEdit size={11} />
                        </button>
                        <button onClick={() => confirmToast("Are you sure you want to delete this product?", () => handleDelete(p._id))} className="p-1.5 rounded border border-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/10 transition">
                          <FaTrash size={11} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/75 flex items-start sm:items-center justify-center z-50 p-3 sm:p-4 overflow-y-auto">
          <div className="w-full max-w-[460px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] overflow-hidden my-auto bg-white dark:bg-[#111111]">
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
              ...
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition">
                <FaTimes />
              </button>
            </div>
            <form onSubmit={handleSave} className="p-4 sm:p-5 space-y-3">
              ...
              <div className="flex gap-2 sm:gap-3 pt-1">
                <button type="button" onClick={closeModal} className="flex-1 py-2 rounded-lg text-sm border border-gray-300 dark:border-[#1f1f1f] text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white bg-transparent transition">
                  Cancel
                </button>
                <button type="submit" className="flex-1 py-2 rounded-lg text-sm font-semibold text-white bg-[#b00000] hover:bg-[#8b0000] transition">
        ...
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="w-full max-w-[400px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] max-h-[90vh] overflow-y-auto bg-white dark:bg-[#111111]">
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between sticky top-0 bg-white dark:bg-[#111111]">
              ...
              <button onClick={() => setViewItem(null)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition ml-2 flex-shrink-0">
                <FaTimes />
              </button>
            </div>
