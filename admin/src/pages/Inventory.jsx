...
const initialProducts = [
  { id: 1, name: "iPhone 13",          imei: "IP13-001", category: "Smartphones", qty: 4,  cost: 200000, price: 250000, supplier: "Apple LK",   description: "Flagship phone",   updated: "2026-04-20" },
  { id: 2, name: "Samsung S24",        imei: "SAM-S24",  category: "Smartphones", qty: 12, cost: 180000, price: 220000, supplier: "Samsung LK", description: "Android flagship", updated: "2026-04-19" },
  { id: 3, name: "iPhone Charger 20W", imei: "CHR-20W",  category: "Charger",     qty: 3,  cost: 3500,   price: 5000,   supplier: "Apple LK",   description: "Original charger", updated: "2026-04-18" },
  { id: 4, name: "AirPods Pro 2",      imei: "APP-PRO2", category: "Airpods",     qty: 8,  cost: 28000,  price: 35000,  supplier: "Apple LK",   description: "ANC earbuds",      updated: "2026-04-17" },
];

const emptyForm = { name: "", imei: "", category: "", qty: "", cost: "", price: "", supplier: "", description: "" };
...
const Inventory = () => {
  ...
  const handleSave = (e) => {
    ...
    if (editItem) {
      setProducts(products.map((p) => p.id === editItem
        ? { ...form, id: editItem, qty: Number(form.qty), cost: Number(form.cost), price: Number(form.price), updated: today }
        : p));
    } else {
  ...
  return (
    <div className="text-gray-900 dark:text-white space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[
          { icon: <FaBox />,                  label: "Total Products",  value: totalProducts,                      color: "text-blue-500 dark:text-blue-400",   border: "border-blue-500/20"  },
          { icon: <FaExclamationTriangle />,  label: "Low Stock",       value: lowStock,                           color: "text-yellow-500 dark:text-yellow-400", border: "border-yellow-500/20" },
          { icon: <FaDollarSign />,           label: "Inventory Value", value: `Rs ${totalValue.toLocaleString()}`, color: "text-green-500 dark:text-green-400",  border: "border-green-500/20"  },
        ].map((s, i) => (
          <div key={i} className={`border ${s.border} rounded-xl p-3 sm:p-4 backdrop-blur-sm bg-white/80 dark:bg-[rgba(17,17,17,0.6)]`}>
            ...
            <div className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-[#1f1f1f] overflow-hidden bg-white/80 dark:bg-[rgba(17,17,17,0.6)] backdrop-blur-sm">
        <div className="px-3 sm:px-4 py-3 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500">{filtered.length} item{filtered.length !== 1 ? "s" : ""}</span>
        </div>

        {/* Mobile card view */}
        <div className="block sm:hidden divide-y divide-gray-100 dark:divide-[#1a1a1a]">
          {filtered.length === 0 ? (
            <div className="text-center py-12 text-gray-400 dark:text-gray-600 text-sm">No products found</div>
          ) : filtered.map((p) => (
            <div key={p.id} className="p-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div>
                  ...
                  <span className="px-1.5 py-0.5 rounded text-[10px] border border-gray-200 dark:border-[#2a2a2a] text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-white/[0.03]">{p.category}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="text-gray-500 dark:text-gray-400">{p.updated}</div>
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button onClick={() => setViewItem(p)} className="flex-1 py-1.5 rounded border border-blue-500/20 text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 transition text-xs flex items-center justify-center gap-1">
                  <FaEye size={10} /> View
                </button>
                <button onClick={() => openEdit(p)} className="flex-1 py-1.5 rounded border border-yellow-500/20 text-yellow-500 dark:text-yellow-400 hover:bg-yellow-500/10 transition text-xs flex items-center justify-center gap-1">
                  <FaEdit size={10} /> Edit
                </button>
                <button onClick={() => handleDelete(p.id)} className="flex-1 py-1.5 rounded border border-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/10 transition text-xs flex items-center justify-center gap-1">
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full text-sm" style={{ minWidth: "700px" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="9" className="text-center py-12 text-gray-400 dark:text-gray-600">No products found</td></tr>
              ) : filtered.map((p) => (
                <tr key={p.id} className="border-t border-gray-100 dark:border-[#1a1a1a] hover:bg-gray-50 dark:hover:bg-white/[0.02] transition">
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{p.name}</td>
                  <td className="px-4 py-3 text-gray-400 font-mono text-xs">{p.imei}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 rounded text-xs border border-gray-200 dark:border-[#2a2a2a] text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-white/[0.03]">{p.category}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-gray-400 dark:text-gray-500 text-xs">{p.updated}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      <button onClick={() => setViewItem(p)} className="p-1.5 rounded border border-blue-500/20 text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 transition"><FaEye size={11} /></button>
                      <button onClick={() => openEdit(p)}    className="p-1.5 rounded border border-yellow-500/20 text-yellow-500 dark:text-yellow-400 hover:bg-yellow-500/10 transition"><FaEdit size={11} /></button>
                      <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded border border-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/10 transition"><FaTrash size={11} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/75 flex items-start sm:items-center justify-center z-50 p-3 sm:p-4 overflow-y-auto">
          <div className="w-full max-w-[460px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] overflow-hidden my-auto bg-white dark:bg-[#111111]">
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-gray-900 dark:text-white">{editItem ? "Edit Product" : "Add Product"}</h2>
                <p className="text-xs text-gray-500 mt-0.5">{editItem ? "Update product details" : "Fill in product information"}</p>
              </div>
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition"><FaTimes /></button>
            </div>
            <form onSubmit={handleSave} className="p-4 sm:p-5 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold tracking-wider block mb-1">NAME *</label>
                  <input required placeholder="Product name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} />
                </div>
                <div>
                  <label className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold tracking-wider block mb-1">imei *</label>
                  <input required placeholder="e.g. IP13-001" value={form.imei} onChange={(e) => setForm({ ...form, imei: e.target.value })} className={inputCls} />
                </div>
              </div>
              <div>
                <label className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold tracking-wider block mb-1">CATEGORY *</label>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {[["QTY *","qty","0"],["COST (Rs) *","cost","0"],["PRICE (Rs) *","price","0"]].map(([label, key, ph]) => (
                  <div key={key}>
                    <label className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold tracking-wider block mb-1">{label}</label>
                    <input required type="number" placeholder={ph} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className={inputCls} />
                  </div>
                ))}
              </div>
              <div>
                <label className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold tracking-wider block mb-1">DESCRIPTION</label>
                <textarea rows="2" placeholder="Optional notes..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={`${inputCls} resize-none`} />
              </div>
        ...
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="w-full max-w-[400px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] max-h-[90vh] overflow-y-auto bg-white dark:bg-[#111111]">
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between sticky top-0 bg-white dark:bg-[#111111]">
              ...
              <button onClick={() => setViewItem(null)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition ml-2 flex-shrink-0"><FaTimes /></button>
            </div>
            <div className="p-4 sm:p-5 space-y-2 sm:space-y-3">
              {[
                ...
                <div key={k} className="flex justify-between items-center text-sm py-1 border-b border-gray-100 dark:border-[#1a1a1a] gap-2">
