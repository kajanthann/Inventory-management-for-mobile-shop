...
const Sales = () => {
  ...
  const addToCart = (product) => {
    setCartItems((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, imei: product.imei, qty: 1, unitPrice: product.price, discount: 0, subtotal: product.price }];
    });
  };
  ...
  const filteredSales = sales.filter((s) =>
    (histSearch === "" || s.invoiceNo.toLowerCase().includes(histSearch.toLowerCase()) ||
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-5 gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 max-h-[260px] sm:max-h-[300px] overflow-y-auto">
                  {filteredProducts.map((p) => (
                    <button
                      ...
                      disabled={p.stock === 0}
                      className="flex items-center justify-between p-3 rounded-lg border text-left transition disabled:opacity-40 disabled:cursor-not-allowed
              ...
              <div className={cardCls}>
                <div className="px-3 sm:px-4 py-3 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <FaShoppingCart size={13} className="text-[#b00000]" />
                    Cart
                  </div>
                          ...
                          <tr key={item.id} className="border-t border-gray-100 dark:border-[#1a1a1a]">
                            ...
                            <td className="px-3 sm:px-4 py-2">
                              <div className="flex items-center justify-center gap-1.5 sm:gap-2">
                                <button onClick={() => updateQty(item.id, -1)}
                                  className="w-5 h-5 rounded flex items-center justify-center border border-gray-300 dark:border-[#2a2a2a] text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:border-[#b00000] transition">
                                  <FaMinus size={8} />
                                </button>
                            ...
                            <td className="px-3 sm:px-4 py-2 text-right">
                              <input
                                ...
                                className="w-16 sm:w-20 px-2 py-1 rounded text-xs text-right outline-none transition border focus:border-[#b00000]
                                  bg-gray-100 dark:bg-[#0a0a0a] border-gray-300 dark:border-[#1f1f1f] text-gray-900 dark:text-white"
                              />
                            </td>
            ...
            <div className={`lg:col-span-2 space-y-4 ${mobilePanel === "products" ? "hidden lg:block" : "block"}`}>
              ...
              <div className={`${cardCls} p-3 sm:p-4 space-y-3`}>
                ...
                <input placeholder="Customer name" value={customer.name}    onChange={(e) => setCustomer({ ...customer, name: e.target.value })}    className={inputCls} />
                <input placeholder="Cashier name"  value={customer.cashier} onChange={(e) => setCustomer({ ...customer, cashier: e.target.value })} className={inputCls} />
                <input placeholder="Phone number"  value={customer.phone}   onChange={(e) => setCustomer({ ...customer, phone: e.target.value })}   className={inputCls} />
              </div>
      ...
      {tab === "history" && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">{filteredSales.length} record{filteredSales.length !== 1 ? "s" : ""}</span>
          </div>
              ...
              <table className="w-full text-sm" style={{ minWidth: "640px" }}>
                ...
                <tbody>
                  {filteredSales.length === 0 ? (
                    <tr><td colSpan="8" className="text-center py-12 text-gray-400 dark:text-gray-600">No sales found</td></tr>
                  ) : filteredSales.map((s) => (
                    <tr key={s.id} className="border-t border-gray-100 dark:border-[#1a1a1a] hover:bg-gray-50 dark:hover:bg-white/[0.02] transition">
                      <td className="px-3 sm:px-4 py-3 font-mono text-xs font-semibold whitespace-nowrap text-[#b00000] dark:text-[#e05050]">{s.invoiceNo}</td>
                      ...
                      <td className="px-3 sm:px-4 py-3 text-right text-xs font-bold text-green-600 dark:text-green-400 whitespace-nowrap">{Rs(s.grandTotal)}</td>
                      <td className="px-3 sm:px-4 py-3">
                      ...
                      <td className="px-3 sm:px-4 py-3">
                        <button onClick={() => setViewSale(s)} className="p-1.5 rounded border border-blue-500/20 text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 transition">
                          <FaEye size={11} />
                        </button>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="w-full max-w-[420px] rounded-xl border border-gray-200 dark:border-[#1f1f1f] overflow-hidden bg-white dark:bg-[#111111]">
            ...
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between">
              ...
              <button onClick={() => setSuccessInvoice(null)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition ml-2"><FaTimes /></button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="w-full max-w-[460px] max-h-[90vh] overflow-y-auto rounded-xl border border-gray-200 dark:border-[#1f1f1f] bg-white dark:bg-[#111111]">
            ...
            <div className="px-4 sm:px-5 py-4 border-b border-gray-200 dark:border-[#1f1f1f] flex items-center justify-between sticky top-0 bg-white dark:bg-[#111111]">
              ...
              <button onClick={() => setViewSale(null)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition"><FaTimes /></button>
            </div>
