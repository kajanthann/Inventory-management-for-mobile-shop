...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const kpi = {
      today: {
        revenue: revenueIn(completedSales, todayStart),
        profit: profitIn(completedSales, todayStart, allProducts),
        sales: completedSales.filter((s) => new Date(s.createdAt) >= todayStart)
          .length,
      },
      week: {
        revenue: revenueIn(completedSales, weekStart),
        profit: profitIn(completedSales, weekStart, allProducts),
        sales: completedSales.filter((s) => new Date(s.createdAt) >= weekStart)
          .length,
      },
      month: {
        revenue: revenueIn(completedSales, monthStart),
        profit: profitIn(completedSales, monthStart, allProducts),
        sales: completedSales.filter((s) => new Date(s.createdAt) >= monthStart)
          .length,
      },
      year: {
        revenue: revenueIn(completedSales, yearStart),
        profit: profitIn(completedSales, yearStart, allProducts),
        sales: completedSales.filter((s) => new Date(s.createdAt) >= yearStart)
          .length,
      },
      allTime: {
        revenue: revenueIn(completedSales, new Date(0)),
        profit: profitIn(completedSales, new Date(0), allProducts),
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      weeklyMap[key] = {
        week: `W${String(Math.ceil(week.getDate() / 7)).padStart(2, "0")} ${week.toLocaleString("default", { month: "short" })}`,
        revenue: 0,
        profit: 0,
        sales: 0,
      };
    ...
    const payMap = {};
    completedSales.forEach((s) => {
      payMap[s.paymentMethod] = (payMap[s.paymentMethod] || 0) + 1;
    });
    const paymentChart = Object.entries(payMap).map(([name, value]) => ({
      name,
      value,
    }));
    ...
    allProducts.forEach((p) => {
      catMap[p.category] = (catMap[p.category] || 0) + p.qty * p.price;
    });
    const categoryChart = Object.entries(catMap).map(([name, value]) => ({
      name,
      value,
    }));
    ...
    const pendingRepairs = allRepairs
    ...
    allRepairs.forEach((r) => {
      ...
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      if (repairMonthMap[key])
        repairMonthMap[key][r.status] =
          (repairMonthMap[key][r.status] || 0) + 1;
    });
